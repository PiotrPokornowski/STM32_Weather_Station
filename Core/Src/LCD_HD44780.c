...
static inline uint8_t LCD_GetDataPort()
{
    ...
    if (HAL_GPIO_ReadPin(LCD_DB4_GPIO_Port, LCD_DB4_Pin) == GPIO_PIN_SET)
        result |= (1 << 0);
    if (HAL_GPIO_ReadPin(LCD_DB5_GPIO_Port, LCD_DB5_Pin) == GPIO_PIN_SET)
        result |= (1 << 1);
    if (HAL_GPIO_ReadPin(LCD_DB6_GPIO_Port, LCD_DB6_Pin) == GPIO_PIN_SET)
        result |= (1 << 2);
    if (HAL_GPIO_ReadPin(LCD_DB7_GPIO_Port, LCD_DB7_Pin) == GPIO_PIN_SET)
        result |= (1 << 3);
#else
    if (HAL_GPIO_ReadPin(LCD_DB0_GPIO_Port, LCD_DB0_Pin) == GPIO_PIN_SET)
        result |= (1 << 0);
    if (HAL_GPIO_ReadPin(LCD_DB1_GPIO_Port, LCD_DB1_Pin) == GPIO_PIN_SET)
        result |= (1 << 1);
    if (HAL_GPIO_ReadPin(LCD_DB2_GPIO_Port, LCD_DB2_Pin) == GPIO_PIN_SET)
        result |= (1 << 2);
    if (HAL_GPIO_ReadPin(LCD_DB3_GPIO_Port, LCD_DB3_Pin) == GPIO_PIN_SET)
        result |= (1 << 3);
    if (HAL_GPIO_ReadPin(LCD_DB4_GPIO_Port, LCD_DB4_Pin) == GPIO_PIN_SET)
        result |= (1 << 4);
    if (HAL_GPIO_ReadPin(LCD_DB5_GPIO_Port, LCD_DB5_Pin) == GPIO_PIN_SET)
...
#else
    GPIO_InitStruct.Pin = LCD_DB0_Pin | LCD_DB1_Pin | LCD_DB2_Pin | LCD_DB3_Pin | LCD_DB4_Pin | LCD_DB5_Pin | LCD_DB6_Pin | LCD_DB7_Pin;
    GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
    GPIO_InitStruct.Pull = GPIO_NOPULL;
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
#endif
